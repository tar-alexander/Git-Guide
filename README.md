...
## Изменение прошлых коммитов

**git restore --staged <file>** - убрать файл из staging area. Перевод из staged обратно в untracked для новых файлов или в modified для ранее закомиченных

**git restore --staged .** сбросить всю текущую папку из коммита

**git reset --hard <commit hash>** откат к указанному коммиту. HEAD переносится на этот коммит, а все поздние коммиты после него теряются

**git restore <file>** для отката файла к уже закомиченным изменениям

## Просмотр изменений в файлах

**git diff** показывает изменения внесённые между коммитами

@@ -1,2 +1,2 @@ строки в сравнении. Указывает, что в сравнении были использованы две строки, начиная с первой

**git diff --staged** для просмотра изменений в staged файлах

**echo "Вторая строка файла" >> file.txt** добавление строки в файл

**echo "Новая строка" > file.txt** одинарная стрелочка перезапишет содержимое файла

**git diff 1c29af6 HEAD** сравнение 1c29af6 коммита и текущего
...
# ? 1 символ. Будут проигнорированы: файлы fileA.txt и file1.txt
file?.txt
...
**git status --ignored** для отображения в выводе игнорируемых файлов
